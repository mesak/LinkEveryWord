...
// bilingual privacy content (generated by sync script). Fallback import kept for backward compat.
// @ts-ignore
import privacyZh from '../src/content/privacy.zh'
// @ts-ignore
import privacyEn from '../src/content/privacy.en'
// @ts-ignore
import privacySingle from '../src/content/privacy'
...
app.get('/privacy', (c: Context) => {
  const { dict, code } = getLang(c)
  const src = (code === 'en' ? (privacyEn || privacySingle) : (privacyZh || privacySingle)) as string
  return Layout(
    c,
